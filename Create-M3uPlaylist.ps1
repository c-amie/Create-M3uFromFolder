...
<#
    ...
    The output filename of the M3U file will be set to match the value of <BookTitle>.m3u
    ...
    .EXAMPLE
    PS> Create-M3uFromFolder -SourceFolder 'C:\MyFolder' -BookTitle 'The Wind in the Willows' -BookYear 1908 -Thuumbnail 'thumbnail.jpg'
...
function Create-M3uFromFolder {
  ...
  Write-Host " Processing Book: $BookTitle ($BookYear)" -ForegroundColor Green

  $strOut = "#EXTM3U`r`n"
  $strOut += "#EXTALB:$($BookTitle) ($($BookYear))`r`n"
  if ($Artist) {
  ...
  #$strOut | Out-File -FilePath "$($strSource)\$($bookTitle).m3u" -Encoding utf8   # Writes a BOM which won't work in UTF-8 M3U files
  [System.IO.File]::WriteAllLines("$($SourceFolder)\$($BookTitle).m3u", $strOut)   # Does not write a BOM
